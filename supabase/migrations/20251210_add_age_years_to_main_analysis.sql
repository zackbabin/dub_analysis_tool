-- Migration: Add age_years to main_analysis materialized view
-- Created: 2025-12-10
-- Purpose: Include age_years from subscribers_insights so we can calculate average age
...
SELECT
  ...
  -- Boolean flags for filtering (keep existing ones)
...
-- Index for age filtering (for average age calculations)
CREATE INDEX IF NOT EXISTS idx_main_analysis_age_years ON main_analysis (age_years) WHERE age_years IS NOT NULL;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Added age_years to main_analysis materialized view';
  RAISE NOTICE '   - Pulled from subscribers_insights.age_years';
  RAISE NOTICE '   - Added index for efficient age filtering';
  RAISE NOTICE '   - Enables average age calculations in Summary Stats';
