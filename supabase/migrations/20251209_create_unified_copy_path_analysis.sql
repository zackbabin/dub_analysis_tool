-- Create unified copy path analysis combining creator and portfolio views
-- Shows combined viewing patterns (creators + portfolios) before first copy
-- Similar to subscription path analysis but for copy conversions

-- ===========================================
-- Create unified copy path analysis table
-- ===========================================

CREATE TABLE IF NOT EXISTS unified_copy_path_analysis (
...
CREATE INDEX idx_unified_copy_path_analysis_type ON unified_copy_path_analysis(analysis_type);
CREATE INDEX idx_unified_copy_path_analysis_rank ON unified_copy_path_analysis(path_rank);

GRANT SELECT ON unified_copy_path_analysis TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON unified_copy_path_analysis TO service_role;

COMMENT ON TABLE unified_copy_path_analysis IS
  'Unified copy conversion path analysis combining creator and portfolio views.
   Shows most common viewing patterns (creators + portfolios) before first copy.';

-- ===========================================
-- Create unified copy path analysis function
-- ===========================================

CREATE OR REPLACE FUNCTION analyze_unified_copy_paths()
RETURNS TABLE(
  analysis_type TEXT,
  path_rank INT,
  view_sequence TEXT[],
...
DO $$
BEGIN
  RAISE NOTICE '✅ Created unified_copy_path_analysis table and function';
  RAISE NOTICE '   - Combines creator and portfolio views before copy';
  RAISE NOTICE '   - Returns combinations and full_sequence analysis types';
  RAISE NOTICE '   - Deduplicates all occurrences (keeps first only)';
END $$;
