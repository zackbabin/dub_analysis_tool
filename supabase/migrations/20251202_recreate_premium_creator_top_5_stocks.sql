-- Recreate premium_creator_top_5_stocks view after breakdown update
...
'Top 5 stock holdings for each premium creator. Includes total_copies and total_copy_capital for sorting and display. Each stock includes ticker, quantity, and avg_price for percentage calculations. Depends on premium_creator_breakdown.';

-- Log the changes
DO $$
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Recreated premium_creator_top_5_stocks after breakdown update';
  RAISE NOTICE '   - View depends on premium_creator_breakdown (without cancellation_rate)';
  RAISE NOTICE '';
END $$;
