-- Fix Premium Creator Affinity totals calculation
-- Problem: Summing copy_count across all user-portfolio rows inflates totals
-- Solution: Count distinct (user, portfolio) combinations instead

DROP VIEW IF EXISTS premium_creator_affinity_display;
DROP VIEW IF EXISTS premium_creator_copy_affinity_base;

-- Recreate base view with correct totals calculation
...
premium_totals AS (
  ...
  -- Each (distinct_id, portfolio_ticker) combination = 1 copy
  SELECT
    premium_creator,
...
SELECT
  ...
  COUNT(*) AS total_copies  -- Count distinct (user, portfolio) combinations
...
WITH all_premium_creators AS (
  ...
  LEFT JOIN (
    SELECT
      premium_creator,
      MAX(premium_creator_total_copies) AS total_copies,  -- Use MAX instead of SUM since values are repeated
...
'Premium creator affinity analysis. Shows which creators are copied by users who copy premium creators. Totals count distinct (user, portfolio) combinations.';
