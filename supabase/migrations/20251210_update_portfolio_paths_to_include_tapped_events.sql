-- Migration: Update analyze_portfolio_copy_paths to include Tapped Portfolio Card events
-- Created: 2025-12-10
-- Purpose: Include both "Viewed Portfolio Details" and "Tapped Portfolio Card" events in path analysis
--          Distinguish tapped events with "(C)" suffix (e.g., "AAPL(C)")
...
BEGIN
  ...
  ordered_views AS (
    ...
    -- Append "(C)" to tapped events for distinction
    SELECT
      ps.user_id,
      CASE
        WHEN ps.event_name = 'Tapped Portfolio Card' THEN ps.portfolio_ticker || '(C)'
...
Tapped events are distinguished with "(C)" suffix (e.g., "AAPL(C)").
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Updated analyze_portfolio_copy_paths to include Tapped Portfolio Card events';
  RAISE NOTICE '   - Both event types now included in analysis';
  RAISE NOTICE '   - Tapped events distinguished with "(C)" suffix';
  RAISE NOTICE '   - Example: "AAPL" = Viewed, "AAPL(C)" = Tapped from category';
  RAISE NOTICE '';
END $$;
