-- Fix ambiguous column reference in process_raw_events_to_profiles
-- The variable v_events_processed conflicts with column name events_processed
-- Solution: Qualify column references with CTE name
-- Date: 2025-11-17
...
BEGIN
  ...
  SELECT
    ...
    synced_at,
    event_aggregates.events_count_per_user  -- FIXED: Renamed to avoid ambiguity with v_events_processed
  ...
  ON CONFLICT (distinct_id) DO UPDATE SET
    ...
    -- Event metrics: REPLACE with new totals (3-day rolling window)
