-- Migration: Revert did_deposit column addition from main_analysis
-- Created: 2025-11-26
-- Purpose: Remove did_deposit column since we're using continuous variables (total_ach_deposits)
--          for behavioral driver analysis, not binary flags
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Reverted main_analysis to remove did_deposit column';
  RAISE NOTICE '   - Behavioral driver analysis uses continuous variables:';
  RAISE NOTICE '     • total_copies (copy drivers)';
  RAISE NOTICE '     • total_ach_deposits (deposit drivers)';
  RAISE NOTICE '     • total_subscriptions (subscription drivers)';
  RAISE NOTICE '   - Kept did_copy and did_subscribe for filtering';
