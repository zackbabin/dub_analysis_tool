-- Restore full working version of premium_creator_top_5_stocks view
-- This includes allocation % ranking and avg_price that was missing in 20251124 recreation
-- Date: 2025-11-25
...
'Top 5 stocks for each premium creator ranked by allocation % of total copy capital. Calculation: (quantity * avg_price) / copy_capital. Shows stocks with highest percentage allocations. Includes avg_price for each stock. Depends on premium_creator_breakdown and portfolio_stock_holdings.';

-- Log success
DO $$
BEGIN
  RAISE NOTICE '===============================================';
  RAISE NOTICE 'Restored full premium_creator_top_5_stocks view!';
  RAISE NOTICE '  ✓ Includes allocation %% ranking';
  RAISE NOTICE '  ✓ Includes avg_price in output';
  RAISE NOTICE '  ✓ Includes total_copy_capital for calculations';
  RAISE NOTICE '---';
  RAISE NOTICE 'This view is a regular VIEW (not materialized)';
  RAISE NOTICE 'No refresh needed - data is always current';
  RAISE NOTICE '===============================================';
END $$;
