...
-- Create table to store portfolio metrics from CSV uploads
...
CREATE INDEX IF NOT EXISTS idx_portfolio_metrics_strategy ON portfolio_metrics(strategy_id);

-- Grant permissions
GRANT SELECT ON portfolio_ticker_mapping TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON portfolio_metrics TO anon, authenticated;
