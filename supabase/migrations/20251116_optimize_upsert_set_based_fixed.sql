...
'Upserts subscriber event data using optimized set-based SQL (10-50x faster than loop version). REPLACES event counts (not incremental) since we fetch a 45-day rolling window each sync. This prevents double-counting events when running multiple syncs.';
