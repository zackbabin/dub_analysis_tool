-- Migration: Fix portfolio copy paths analysis types to match frontend expectations
-- Created: 2025-12-10
-- Purpose: Update analyze_portfolio_copy_paths to return analysis types that match frontend display
--
-- Issue: Portfolio function returns 'first_portfolio' and 'last_portfolio' but frontend expects
--        'top_portfolios_viewed' and 'portfolio_combinations' to match creator function pattern
--
-- Solution: Update to match creator function pattern:
--   - top_portfolios_viewed: Most viewed portfolios (by total view count)
--   - portfolio_combinations: Common portfolio combinations (unordered sets)
--   - full_sequence: Complete sequential paths (preserving order)
...
BEGIN
  ...
  -- Top 10 complete sequences (last 5 portfolios before copy, preserving order)
  user_sequences AS (
    SELECT
      user_id,
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed portfolio copy paths analysis types';
  RAISE NOTICE '   - Changed first_portfolio → top_portfolios_viewed';
  RAISE NOTICE '   - Changed last_portfolio → portfolio_combinations';
  RAISE NOTICE '   - Now matches creator function pattern and frontend expectations';
  RAISE NOTICE '';
END $$;
