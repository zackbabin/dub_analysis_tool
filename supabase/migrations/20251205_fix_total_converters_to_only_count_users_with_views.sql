-- Migration: Fix total_converters to only count users with views
-- Created: 2025-12-05
-- Purpose: total_converters should only count users who have views between first_app_open and first_copy
--          Currently counts all 10,350 users with both timestamps, but only 1,505 have portfolio views
--          and only 511 have creator views, making percentages misleading
...
BEGIN
  ...
  user_sequences AS (
    ...
    FROM deduped_views
    WHERE portfolio_ticker IS DISTINCT FROM prev_portfolio
    GROUP BY user_id
  ),
...
BEGIN
  ...
  user_sequences AS (
    ...
    FROM deduped_views
    WHERE creator_username IS DISTINCT FROM prev_creator
    GROUP BY user_id
  ),
...
BEGIN
  ...
  RAISE NOTICE '   - Portfolio: Now counts ~1,505 users (14.54%% of 10,350) instead of all 10,350';
  RAISE NOTICE '   - Creator: Now counts ~511 users (4.94%% of 10,350) instead of all 10,350';
