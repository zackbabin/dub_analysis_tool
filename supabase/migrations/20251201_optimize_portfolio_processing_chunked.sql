-- Migration: Optimize portfolio engagement processing with chunked processing
-- Created: 2025-12-01
-- Purpose: Process staging data in chunks to avoid statement timeout with large datasets
--
-- Problem: Single INSERT...SELECT with ON CONFLICT can timeout on large datasets (70k+ records)
-- Solution: Process in chunks of 10k records at a time
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Optimized process_portfolio_engagement_staging() function';
  RAISE NOTICE '   - Now processes in chunks of 10k records';
  RAISE NOTICE '   - Prevents statement timeout on large datasets (70k+ records)';
