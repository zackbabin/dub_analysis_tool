-- Change upsert_subscribers_incremental to REPLACE event counts instead of ADD
-- Since we now fetch a 60-day rolling window each time, we want to replace totals
-- rather than incrementally add, to avoid double-counting the same events
-- Date: 2025-11-15
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT (distinct_id) DO UPDATE SET
      ...
      -- Event metrics: REPLACE with new totals (since we're fetching 60-day window each time)
      -- The new values represent the complete totals for the rolling 60-day window
...
'Upserts subscriber event data. REPLACES event counts (not incremental) since we fetch a 60-day rolling window each sync. This prevents double-counting events when running multiple syncs.';
