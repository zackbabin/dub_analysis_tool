-- Migration: Fix creator path_rank type mismatch
-- Created: 2025-12-10
-- Purpose: Change path_rank from BIGINT to INT to match table schema and portfolio function
...
RETURNS TABLE(
  analysis_type TEXT,
  path_rank INT,  -- Changed from BIGINT to INT
...
BEGIN
  ...
  -- Top 10 complete sequences (last 5 creators before copy, preserving order)
  user_sequences AS (
    SELECT
      user_id,
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed creator path_rank type mismatch';
  RAISE NOTICE '   - Changed path_rank from BIGINT to INT';
  RAISE NOTICE '   - Now consistent with table schema and portfolio function';
  RAISE NOTICE '';
END $$;
