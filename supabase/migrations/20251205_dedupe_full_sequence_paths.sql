-- Migration: Deduplicate consecutive items in full sequence paths
-- Created: 2025-12-05
-- Purpose: Remove consecutive duplicate views (e.g., $PELOSI → $PELOSI → $PELOSI becomes $PELOSI)

-- ===========================================
-- 1. Fix Portfolio Full Sequence Deduplication
-- ===========================================
...
BEGIN
  ...
  -- Deduplicate consecutive items in sequences
  deduped_views AS (
    SELECT
      user_id,
      portfolio_ticker,
      event_time,
      position_from_end,
      LAG(portfolio_ticker) OVER (PARTITION BY user_id ORDER BY event_time ASC) as prev_portfolio
    FROM ordered_views
  ...
  user_sequences AS (
    ...
    FROM deduped_views
    WHERE portfolio_ticker IS DISTINCT FROM prev_portfolio  -- Remove consecutive duplicates
    GROUP BY user_id
  ),
...
-- ===========================================
-- 2. Fix Creator Full Sequence Deduplication
-- ===========================================
...
BEGIN
  ...
  deduped_views AS (
    SELECT
      ...
      event_time,
      position_from_end,
      LAG(creator_username) OVER (PARTITION BY user_id ORDER BY event_time ASC) as prev_creator
    FROM ordered_views
    WHERE position_from_end <= 5
  ),
  ...
  user_sequences AS (
    ...
    FROM deduped_views
    WHERE creator_username IS DISTINCT FROM prev_creator  -- Remove consecutive duplicates
    GROUP BY user_id
  ),
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed full sequence deduplication';
  RAISE NOTICE '   - Added LAG() window function to detect consecutive duplicates';
  RAISE NOTICE '   - Filter: WHERE ticker/username IS DISTINCT FROM prev_ticker/username';
  RAISE NOTICE '   - Example: [$PELOSI, $PELOSI, $SIMBA] → [$PELOSI, $SIMBA]';
  RAISE NOTICE '';
END $$;
