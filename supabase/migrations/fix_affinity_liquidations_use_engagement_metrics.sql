-- Fix Premium Creator Affinity to use portfolio_creator_engagement_metrics for liquidations
-- Problem: Affinity view uses premium_creator_portfolio_metrics_latest (Mixpanel chart data)
--          while Breakdown view uses portfolio_creator_engagement_metrics (user-level aggregation)
--          This causes liquidation discrepancies between the two views
-- Solution: Both views should use the same source - portfolio_creator_engagement_metrics
-- Date: 2025-11-11

-- Drop validation views that depend on premium_creator_affinity_display
DROP VIEW IF EXISTS validation_liquidations_comparison;
DROP VIEW IF EXISTS validation_copies_comparison;

-- Drop affinity views
DROP VIEW IF EXISTS premium_creator_affinity_display;
DROP VIEW IF EXISTS premium_creator_copy_affinity_base;

-- Recreate base view with liquidations from portfolio_creator_engagement_metrics
...
premium_totals AS (
  -- Get aggregated totals from portfolio_creator_engagement_metrics
  -- This is the SAME source used by premium_creator_breakdown
...
'Premium creator affinity analysis. Uses portfolio_creator_engagement_metrics for copy/liquidation totals (same source as premium_creator_breakdown to ensure consistency).';

-- Recreate validation views that were dropped
