-- Final comprehensive fix for all metric aggregation issues
-- Date: 2025-11-03
-- Purpose:
--   1. Use aggregated totals from premium_creator_portfolio_metrics (not user-level sums)
--   2. Fix premium creator affinity to use aggregated totals
--   3. Ensure all views are consistent and correct
...
-- ============================================================================
-- STEP 2: Recreate portfolio_creator_engagement_metrics with correct aggregation
-- ============================================================================
...
-- ============================================================================
-- STEP 3: Recreate hidden_gems_portfolios
-- ============================================================================
...
-- ============================================================================
-- STEP 4: Recreate premium creator affinity with aggregated totals
-- ============================================================================
...
premium_totals AS (
  ...
  SELECT
    pc.creator_username AS premium_creator,
    COALESCE(SUM(pcpm.total_copies), 0) AS total_copies,
    COALESCE(SUM(pcpm.total_liquidations), 0) AS total_liquidations
  FROM premium_creators_list pc
  CROSS JOIN LATERAL unnest(pc.creator_ids) AS pc_creator_id
  LEFT JOIN premium_creator_portfolio_metrics_latest pcpm
...
'Aggregated portfolio-creator engagement metrics. Uses aggregated totals from premium_creator_portfolio_metrics (portfolio-level) and premium_creator_metrics (creator-level). Refresh after sync.';

COMMENT ON MATERIALIZED VIEW hidden_gems_portfolios IS
'Hidden gem portfolios: many unique viewers but few unique copiers (ratio >= 5). Indicates high interest but low conversion. Uses aggregated copy totals from premium_creator_portfolio_metrics.';
