-- Migration: Fix premium_creator_metrics to have one row per creator
-- Created: 2025-12-03
-- Purpose: Remove historical records design, keep only latest metrics per creator
--
-- Changes:
-- 1. Drop existing UNIQUE constraint on (creator_id, synced_at)
-- 2. Add UNIQUE constraint on creator_id only
-- 3. Clean up duplicate rows, keeping most recent
-- 4. Update views to remove MAX() aggregation

-- Step 1: Clean up duplicates - keep only most recent row per creator
...
DROP CONSTRAINT IF EXISTS premium_creator_metrics_creator_id_synced_at_key;

-- Step 3: Add new unique constraint on creator_id only
ALTER TABLE premium_creator_metrics
ADD CONSTRAINT premium_creator_metrics_creator_id_key UNIQUE (creator_id);

-- Step 4: Update premium_creator_breakdown view to remove MAX() since now one row per creator
...
subscription_by_username AS (
    -- Changed: No more MAX() since one row per creator
    SELECT pc.creator_username,
        pcm.total_subscriptions,
        pcm.total_paywall_views,
        pcm.total_cancellations
    FROM premium_creators pc
    LEFT JOIN premium_creator_metrics pcm
        ON pc.creator_id = pcm.creator_id
    -- Note: No GROUP BY needed since one row per creator_id
),
...
'Creator-level aggregated metrics for Premium Creator Breakdown. Regular view (not materialized) that always shows fresh data. Now uses single row per creator from premium_creator_metrics (no MAX needed).';

-- Log the changes
DO $$
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed premium_creator_metrics to have one row per creator';
  RAISE NOTICE '   - Cleaned up duplicate rows (kept most recent)';
  RAISE NOTICE '   - Changed UNIQUE constraint from (creator_id, synced_at) to (creator_id)';
  RAISE NOTICE '   - Updated premium_creator_breakdown view to remove MAX()';
  RAISE NOTICE '';
END $$;
