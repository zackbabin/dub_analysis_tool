-- Migration: Reduce portfolio engagement chunk size for better timeout handling
-- Created: 2025-12-01
-- Purpose: Reduce chunk size from 10k to 5k records to prevent statement timeout
--
-- Issue: Even with chunking, 10k record chunks are timing out on large datasets
-- Solution: Reduce to 5k records per chunk for faster processing
...
DECLARE
  ...
  v_chunk_size integer := 5000;  -- Reduced from 10k to 5k records per chunk
  v_offset integer := 0;
...
BEGIN
  ...
  LOOP
    ...
    WITH chunk AS (
      ...
      ORDER BY user_id, portfolio_ticker, creator_id
      LIMIT v_chunk_size
      OFFSET v_offset
    )
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ Reduced chunk size for process_portfolio_engagement_staging()';
  RAISE NOTICE '   - Reduced from 10k to 5k records per chunk';
  RAISE NOTICE '   - Should prevent statement timeout on large datasets';
  RAISE NOTICE '';
END $$;
