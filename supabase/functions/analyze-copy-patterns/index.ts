// Supabase Edge Function: analyze-copy-patterns
// Analyzes PORTFOLIO copy patterns using exhaustive search + logistic regression
// Tests portfolio combinations to find which pairs of portfolios drive copies
// Runs independently from analyze-creator-copy-patterns to avoid CPU timeout
...
const ANALYSIS_CONFIG = {
  ...
  entityIdColumn: 'portfolio_ticker',
  analysisType: 'copy'  // Hardcoded for this function
}
...
serve(async (req) => {
  ...
  try {
    // Hardcoded to 'copy' analysis type (portfolio combinations)
    const config = ANALYSIS_CONFIG
    const analysisType = config.analysisType
    console.log(`Starting ${analysisType} pattern analysis (portfolio combinations)...`)
